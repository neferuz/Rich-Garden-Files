...
export default function OrderDetails({ order, isModal = false, onClose }: OrderDetailsProps) {
    ...
    const getStatusInfo = (status: string) => {
        switch (status) {
            case 'new': return { label: 'Новый', color: 'text-blue-600', bg: 'bg-blue-100', icon: AlertCircle } // Blue for New per design
    ...
    const containerClasses = isModal
        ? "fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-end justify-center sm:items-center p-0 sm:p-4 animate-in fade-in duration-200"
        : "min-h-screen bg-gray-50/50 pb-32 font-sans"

    const contentClasses = isModal
        ? "w-full sm:max-w-md max-h-[90vh] bg-[#F2F3F5] overflow-y-auto rounded-t-[32px] sm:rounded-[32px] animate-in slide-in-from-bottom duration-300 shadow-2xl relative"
        ...
        <div className={containerClasses} onClick={isModal ? onClose : undefined}>
            <div className={contentClasses} onClick={e => e.stopPropagation()}>
                ...
                <div className="bg-white/80 backdrop-blur-md px-6 py-4 sticky top-0 z-20 flex items-center justify-between border-b border-gray-100/50">
                    {isModal ? (
                        <button onClick={onClose} className="w-10 h-10 flex items-center justify-center rounded-full bg-white border border-gray-200 text-gray-500 hover:bg-gray-100 transition-colors">
                            <X size={20} />
                        </button>
                    ) : (
                    ...
                    <div className="bg-white p-5 rounded-[24px] shadow-sm">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <div className="flex items-center gap-2">
                                <a href={`tel:${order.clientPhone}`} className="w-9 h-9 rounded-full bg-green-50 text-green-600 flex items-center justify-center hover:bg-green-100 transition-colors">
                                    <Phone size={18} />
                                </a>
                            </div>
                        </div>
                            ...
                            <Link href={`/clients/${order.userId}`} className="flex items-center justify-between mb-5 hover:bg-gray-50 -mx-2 px-2 py-2 rounded-xl transition-colors group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-medium text-gray-400 mt-1">{order.clientPhone}</p>
                                    </div>
                                </div>
                        ...
                        <div className="space-y-4">
                            {order.items.map((item, i) => (
                                <div key={i} className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="w-14 h-14 rounded-2xl bg-gray-50 border border-gray-100 p-1 flex items-center justify-center shrink-0 relative overflow-hidden">
                                            {item.image ? (
                        ...
                        <div className="relative pl-3 space-y-6">
                            ...
                            <div className="relative flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs font-medium text-gray-400">{order.time}</p>
                                </div>
                            </div>
                            <div className="relative flex items-center gap-4">
                                <div className="w-3 h-3 rounded-full bg-blue-500 ring-4 ring-white relative z-10 shrink-0"></div>
                                <div>
                                    <p className="text-sm font-bold text-gray-900">Оплачен</p>
                                    <p className="text-xs font-medium text-gray-400">10:20</p>
                                </div>
                            </div>
                        </div>
                    </div>
